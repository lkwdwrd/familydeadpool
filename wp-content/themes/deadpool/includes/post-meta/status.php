...
namespace Deadpool\Post_Meta\Status;
...
function display_metabox( $post ) {
	$current_value = Helpers\get_char_status( $post );

	// Output the nonce
	wp_nonce_field( 'char_status_noncy_' . $post->ID, 'char_status_nonce' );
	...
	<p>
		<input
			type="radio"
			name="char_status"
			id="char_status_alive"
			value="alive"
			<?php checked( 'alive', $current_value ); ?>
		/>
		<label for="char_status_alive">
			<?php esc_html_e( 'Alive', 'die' ); ?>
		</label>
	</p>
	<p>
		<input
			...
			id="char_status_dead"
			value="dead"
			<?php checked( 'dead', $current_value ); ?>
		/>
		<label for="char_status_dead">
			<?php esc_html_e( 'Dead', 'die' ); ?>
		</label>
	</p>
	<?php
}
...
function save( $post_id ) {
	...
	// Verify nonce
	$nonce_name = 'char_status_nonce';
	$nonce_action = 'char_status_noncy_' . $post_id;
	...
	// Save data
	// @codingStandardsIgnoreLine
	$value = $_POST[ 'char_status' ] ?? null;
	if ( null !== $value ) {
		// Whitelist values to sanitize.
		$value = 'dead' === $value ? 'dead' : 'alive';
		update_post_meta( $post_id, 'char_status', $value );
	} else {
		delete_post_meta( $post_id, 'char_status' );
	}
}
...
/**
 * Gets the post types that support the status post meta.
 *
 * @return array The post types that support this metabox.
 */
function get_post_types() {
	return apply_filters( 'char_status_meta', [] );
}
